...
const calculateRelativeFrequency = (text: string) => {
  const frequencies: { [key: string]: number } = {};
  const totalLetters = text.length;

  // Initialize frequencies with 0 for each letter
  for (let i = 0; i < 26; i++) {
    frequencies[String.fromCharCode(i + 65)] = 0;
  }
...
export default function AutokeyVigenereCipher() {
  ...
  // State untuk freq
  const [relativeFrequencies, setRelativeFrequencies] = useState<{ [key: string]: number } | null>(null);
  ...
  return (
    <div className="flex h-[100%]">
      <div className="w-[45%] font-mono font-medium border-black border-l-4 border-t-4 border-b-4 border-r-2 h-full">
          ...
          <div className="space-y-2 w-full">
            ...
            {inputType == "text" && (
              <div className="flex">
                ...
                <button onClick={encryptAutokeyVigenereCipher} className="p-2 bg-white text-black outline outline-3 outline-black rounded-lg hover:bg-black hover:text-white focus:bg-black focus:text-white">
                  Encrypt Text
                </button>
                <button onClick={decryptAutokeyVigenereCipher} className="ml-4 p-2 bg-white text-black outline outline-3 outline-black rounded-lg hover:bg-black hover:text-white">
                  Decrypt Text
                </button>
              </div>
            )}
            ...
            {inputType == "file" && (
              <div className="flex">
                ...
                <button onClick={decryptAutokeyVigenereCipher} className="ml-4 p-2 bg-white text-black outline outline-3 outline-black rounded-lg hover:bg-black hover:text-white focus:bg-black focus:text-white">
                  Decrypt File
                </button>
              </div>
            )}
        ...
        <div className="p-4 border-b-4 border-black bg-[#b9924a]">
          <div className="font-mono md:text-md lg:text-xl tracking-tighter font-semibold">18221049 Silvester Kresna W. P. P.</div>
        </div>
        <div className="flex items-center justify-center text-center pt-4 px-4 md:text-5xl lg:text-5xl font-reggae">Autokey Vigenere</div>
